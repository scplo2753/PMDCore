...
/******
 * @brief return splited MD tag, The MD tag is split into numbers and non-numbers, for example, "10A5^C3" will be split
 * into ["10", "A", "5", "^C", "3"].
...
parsedRecordError parsedData::parseCIGAR(const recordLine_struct_t& raw_data, CIGARList_t& output)
{
    ...
    while (pos < n)
    {
        ...
        while (pos < n && cigar[pos] >= '0' && cigar[pos] <= '9') ++pos;
...
std::vector<std::size_t> parsedData::getOpListInCIGAR(char Op)
{
    ...
    for (auto& line : cigar_list)
    {
        ...
        if (line.first == Op)
        {
            for (std::size_t temp = current_pos; temp < (current_pos + step_length); temp++) Op_list.emplace_back(temp);
        }
